...
// SPReAD-1000 clinical-science 03: chemistry molecular generation (REINVENT4)
//
// リソース:
...
param storageAccountName string = toLower('stmolgen${uniqueString(resourceGroup().id)}')

@description('Key Vault 名 (3-24 chars)')
param keyVaultName string = 'kv-molgen-${uniqueString(resourceGroup().id)}'

@description('Application Insights 名')
param appInsightsName string = 'ai-molgen-${uniqueString(resourceGroup().id)}'

@description('ACR 名 (5-50 chars, 英数のみ, globally unique)')
param acrName string = toLower('crmolgen${uniqueString(resourceGroup().id)}')

@description('Log Analytics Workspace 名 (Application Insights のバックエンド)')
param logAnalyticsName string = 'log-molgen-${uniqueString(resourceGroup().id)}'
...
// このシナリオは REINVENT prior を AML の workspaceblobstore (auto-created container)
// にアップロードするため、追加コンテナは作成しない。
...
// (prior ファイルを local から blob へアップロードするため)
// ----------------------------------------------------------------------------
