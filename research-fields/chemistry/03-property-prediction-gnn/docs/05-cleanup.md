...
```bash
# 消える対象を最終確認
az resource list -g spread-chem-gnn-rg -o table

# 実行
az group delete -n spread-chem-gnn-rg --yes --no-wait
```
...
```bash
az group show -n spread-chem-gnn-rg 2>&1 | grep -i "not found" && \
...
Azure Portal → **Cost Management + Billing** → **コスト分析** → スコープを本サブスクリプションに、フィルタで **リソースグループ = spread-chem-gnn-rg** を指定すると、この試行の合計コストが確認できます。**$1 以下**に収まっていれば想定通りです。
