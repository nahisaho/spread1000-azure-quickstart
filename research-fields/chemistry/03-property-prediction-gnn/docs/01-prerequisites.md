...
`deploy.sh` は **Bash 前提**です。次のいずれかで実行してください：
...
T4 GPU (NCasT4_v3) は次のリージョンで在庫が安定しています：
...
> [!NOTE]
> このパッケージ群は**ローカルの verify-output.py 用**です。ジョブ側 (Azure ML コンピュート) の依存関係は `aml/conda.yml` で管理されています。
