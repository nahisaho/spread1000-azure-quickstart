...
> [!IMPORTANT]
> 生成される PDF は**すべて架空**（人名、事件番号、企業名、住所すべて捏造）です。実在の判決や企業情報を含めていません。
> The demo PDFs generated by scripts/generate_demo_pdfs.py are dedicated to the public domain under CC0 1.0: https://creativecommons.org/publicdomain/zero/1.0/legalcode
...
### 不可逆的マスキング (実文書を使う場合は必須)

「黒塗り矩形を重ねる」だけではコンテンツが PDF に残ります。以下のいずれかの方法で**不可逆的に除去**してください：

```bash
# 方法 1: pdf-redact-tools (Linux/WSL2)
sudo apt install -y pdf-redact-tools
pdf-redact-tools --input original.pdf --output redacted.pdf

# 方法 2: Python で pdfplumber + pikepdf を使って手動除去
# 参考: https://pikepdf.readthedocs.io/

# 方法 3: Adobe Acrobat / LibreOffice の「コンテンツを完全に削除」機能
```

**マスキング確認 (必須)**:

```bash
# テキストが本当に除去されたか確認 (要 poppler-utils)
pdftotext redacted.pdf - | grep -F "<sensitive-string>" \
  && echo "REDACTION FAILED — content still present" \
  || echo "OK — not found in text layer"
```

> [!WARNING]
> スキャン画像 PDF の場合、`pdftotext` ではテキスト層を検出できません。Doc Intelligence の OCR 結果 (`*.markdown.txt`) でも確認してください。
