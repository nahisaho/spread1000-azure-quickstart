...
> [!CAUTION]
> **本教材の学習済みモデルは診断・治療の判断に一切使用してはいけません。**
> 研究・教育目的の実装例であり、臨床適用には (i) 多施設・患者独立分割による臨床試験、(ii) PMDA/FDA 等の医療機器承認 (SaMD)、(iii) 医療機関内での IRB 審査、(iv) 医師による最終判断プロセスが必須です。本モデルを診断に用いた結果生じた損害について、教材提供者は一切責任を負いません。
