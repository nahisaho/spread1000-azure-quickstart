// ============================================================
// TamGen クイックスタート — Azure ML Workspace + GPU Compute
//
// 使い方:
//   MY_OID=$(az ad signed-in-user show --query id -o tsv)
//   MY_TID=$(az account show --query tenantId -o tsv)
//   az group create -n rg-spread1000-tamgen-drug-discovery-<name> -l japaneast
//   az deployment group create \
//     -g rg-spread1000-tamgen-drug-discovery-<name> \
//     -f main.bicep \
//     -p yourName=<name> ownerEmail=<mail> assignedUserObjectId=$MY_OID assignedUserTenantId=$MY_TID
//
// 前提: Azure ML GPU クォータ (NCadsA100_v4 24 vCPU 以上) 確保済み。
...
@allowed([
  'Standard_NC24ads_A100_v4'   // A100 80GB (推奨)
  'Standard_NC8as_T4_v3'        // T4 16GB (低コスト)
  'Standard_NC40ads_H100_v5'    // H100 80GB (大規模)
])
param computeSize string = 'Standard_NC24ads_A100_v4'
...
var workspaceName = take('mlw-tamgen-${yourName}', 33)
var computeName   = take('ci-tamgen-${yourName}-${suffix}', 24)
var storageName   = take('sttamgen${uniqueString(resourceGroup().id, yourName)}', 24)
var kvName        = take('kv-tamgen-${suffix}-${uniqueString(resourceGroup().id)}', 24)
var appiName      = 'appi-tamgen-${yourName}-${suffix}'
var crName        = take('crtamgen${uniqueString(resourceGroup().id, yourName)}', 50)
...
resource ws 'Microsoft.MachineLearningServices/workspaces@2024-04-01' = {
  ...
  properties: {
    friendlyName: 'TamGen Quickstart (${yourName})'
...
resource ci 'Microsoft.MachineLearningServices/workspaces/computes@2024-04-01' = {
  ...
  properties: {
    ...
    properties: {
      vmSize: computeSize
      idleTimeBeforeShutdown: 'PT60M'  // 60 分アイドルで自動停止
