...
| [03](03-biosignal-ecg-classification/) | **生体信号 (ECG) 分類** | PhysioNet MIT-BIH（Open Data、ODC-By）を AAMI 5-class に分類する 1D CNN 学習（Azure ML v2 command job） | NC4as_T4_v3（CPU fallback: D4as_v5） | $1〜3 (¥150〜450) |
...
3. **生体信号 (ECG) 分類** — MIT-BIH × 1D CNN を AML compute で学習させる公開データ ML 最短ルート

## 想定される SPReAD-1000 課題例（実データより）

- 「医用画像」「CT/MRI 解析」「病変検出」→ シナリオ 01
- 「電子カルテ NLP」「臨床テキスト解析」→ シナリオ 02
- 「生体信号」「心電図」「時系列解析」→ シナリオ 03
