...
// SPReAD-1000 clinical-science 03: biosignal ECG classification
//
// リソース:
...
param storageAccountName string = toLower('stecg${uniqueString(resourceGroup().id)}')

@description('Key Vault 名 (3-24 chars)')
param keyVaultName string = 'kv-ecg-${uniqueString(resourceGroup().id)}'

@description('Application Insights 名')
param appInsightsName string = 'ai-ecg-${uniqueString(resourceGroup().id)}'

@description('ACR 名 (5-50 chars, 英数のみ, globally unique)')
param acrName string = toLower('crecg${uniqueString(resourceGroup().id)}')

@description('Log Analytics Workspace 名 (Application Insights のバックエンド)')
param logAnalyticsName string = 'log-ecg-${uniqueString(resourceGroup().id)}'
...
// (MIT-BIH データを local から blob へアップロードするため)
// ----------------------------------------------------------------------------
