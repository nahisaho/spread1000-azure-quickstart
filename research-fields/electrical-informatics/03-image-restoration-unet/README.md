...
**手法**: 幾何プリミティブ (矩形・円・グラデーション) から生成した合成クリーン画像 + 加算 Gaussian ノイズ × コンパクト U-Net（約 117K パラメータ）
...
# 2. 合成データ生成 (200 train + 40 val, 30 秒)
...
# 4. 評価 (40 val 画像で PSNR/SSIM 計算, 30 秒)
...
| モデル | MiniUNet (3 レベル、~117K params) | D-3 セグメンテーションと同じ構造を回帰に転用 |
